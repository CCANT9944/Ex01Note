...
- **S-Note Undo/Redo Accuracy**: Hardened the History Stack synchronization for the S-Note Canvas. Actions such as changing colours via the Lasso Context Menu or moving/scaling objects are now explicitly committed to the `undoStack` snapshot memory before mutating. This completely eliminates a bug where undoing a text move or color change would skip the action and accidentally delete a previous drawing line instead, ensuring 100% sequential accuracy when reverting canvas states.
