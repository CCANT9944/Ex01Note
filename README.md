...
- **S-Note Undo/Redo Accuracy**: Hardened the History Stack synchronization for the S-Note Canvas. Actions such as changing colours via the Lasso Context Menu, moving/scaling objects, and standard drawing strokes (Pen, Highlighter, Eraser) are now explicitly committed to the `undoStack` snapshot memory before mutating. This completely eliminates bugs where undoing actions would skip specific tool strokes or color edits, ensuring 100% sequential accuracy when reverting canvas states.
