...
- **Flawless Soft-Wrapped Text & Cursor Hit Detection:** Completely eliminated cursor jumping issues when interacting with softly wrapped multi-line text on the S-Note canvas. The custom hit-detection engine now strictly calculates exact visual rows instead of hard newlines, perfectly anchoring your cursor to the exact visual character tapped. Text styles (color, size) now also apply uniformly to the active text block natively without prematurely splitting chunks or breaking the keyboard layout.
...
- **S-Note Render Optimization & Theme Switching:** Completely eliminated UI freezes when toggling between Light and Dark mode. Mathematical stroke coordinates (`toPath()`) are now heavily optimized with a localized memory cache per line, preventing Jetpack Compose from redundantly recalculating tens of thousands of data points on the main UI thread during full-screen recompositions.
